import hashlib, json, copy, time, random
import requests
import bitcoinlib
...
class Blockchain(object):
    ...
    def isValidChain(self, chain):
        ...
        for blockIndex, block in enumerate(chain):
            if blockIndex:
                if self.getBlockID(chain[blockIndex - 1]) != block['previousHash']:
                    return False
            if not self.isValidProof(block, block['nonce']):
                return False

            merkleRoot = self.generateMerkleRoot(block['transactions'])
            if merkleRoot != block['merkleRoot']:
                return False

            for transaction in block['transactions']:
                transactionCopy = copy.copy(transaction)
                transactionCopy.pop("signature")
                isValidSignature = self.verifySignature(
                    transaction['sender'],
                    transaction['signature'],
                    json.dumps(transactionCopy, sort_keys=True)
                )
    ...
    def resolveConflicts(self):
        ...
        for nodeUrl in self.nodes:
            chain = requests.get(nodeUrl + "chain").json()
            chainIsValid = False
            if len(chain) > len(self.chain):
                chainIsValid = self.isValidChain(chain)
            if(chainIsValid):
                self.chain = chain
        pass
